...
const Meals = ({isHome=false, onlyFavourites=false, addFav,deleteMeal,heading="All Recipes"}) =>
{
    ...
    {
        const apiUrl = isHome ? `http://localhost:5001/api/recipes?_limit=3` : `http://localhost:5001/api/recipes`;
        try
        {
    ...
    return (
        <div className="meals-main">
            ...
            <div className="meals">
                {fav.map((meal)=> 
                (<Meal key={meal._id} meal={meal} addFav={addFav} deleteMeal={deleteMeal}/>)
                )}
            </div>
            }
        </div>
